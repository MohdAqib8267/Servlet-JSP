...
**connection with jdbc and show values**
```
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
...
<body> 
	...
	<sql:setDataSource var="db" driver="com.mysql.cj.jdbc.Driver" password="Aqib8267@" url="jdbc:mysql://localhost:3306/trainTicketBooking" user="root" />
	<sql:query var="rs" dataSource="${db}">select * from user</sql:query>
	<c:forEach items="${rs.rows}" var="u">
		<c:out value="${u.name}"></c:out>:<c:out value="${u.email}"></c:out> <br/>
	</c:forEach>
</body>
</html>
```
